...
function initDiscountManagement() {
    ...
    window.saveDiscount = function () {
        ...
        const endpoint = isUpdating ? "/order/updateDiscount" : "/order/addDiscount";
...
async function loadOrders() {
  const res = await fetch('/order/all'); // adjust path
  ...
  order.items.forEach(item => {
    itemsBody.innerHTML += `
      <tr>
        ...
        <td><img src="/uploads/${item.Product_Images[0]}" width="40"></td>
      </tr>
    `;
  });
